 <?php
  ...
  public function ordCategoria($subCatId){
      $consulta=$this->db->query("select * from PRODUCT join CATEGORY on PRODUCT.CATEGORY = '$subCatId' and CATEGORY.ID = '$subCatId';");
    ...
    //Mostrar marcas de la base de datos en el filtro
    public function get_viewBrand(){
    ...
    public function filterBrand($brandID){
      $consulta=$this->db->query("select * from PRODUCT $brandID;");
      while($filas=$consulta->fetch_assoc()){
          $this->productos[]=$filas;
      }
      return $this->productos;
    ...
    public function filterPrice($Min,$Max){
      $consulta=$this->db->query("select * from PRODUCT where PRICE > '$Min' or PRICE < '$Max';");
      while($filas=$consulta->fetch_assoc()){
          $this->productos[]=$filas;
      }
      return $this->productos;
